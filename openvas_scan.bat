...
rem "%~dp0omp_cracked.exe" --host=%Server% --port=%Port% --username=%Username% --password=%Password% --xml="<create_target><name>\"%Proyecto%_%Timestamp%\"</name><hosts>%IP%</hosts><alive_tests>Consider Alive</alive_tests><port_range>T:1-65535</port_range><port_list id=\"%PortList%\"></port_list></create_target>" 1>"%TEMP%\openvas_scan_target_%Timestamp%.txt" 2>NUL
...
if not %ERRORLEVEL% EQU 0 ( echo ---Error. Revise username/password o levante el servicio en %Server% ejecutando: && echo /etc/init.d/openvas-scanner start && echo /etc/init.d/openvas-manager start && echo /etc/init.d/openvas-gsa start && pause && exit )
"%~dp0xml.exe" sel -t -m "create_target_response" -v "@id" "%TEMP%\openvas_scan_target_%Timestamp%.txt" > %TEMP%\openvas_scan_target_id_%Timestamp%.txt"
...
if %ERRORLEVEL% EQU 0 ( echo ---Error--- && pause && exit )
findstr.exe /C:"OK" "%TEMP%\openvas_scan_task_%Timestamp%.txt"
if not %ERRORLEVEL% EQU 0 ( echo ---Error--- && pause && exit )
"%~dp0xml.exe" sel -t -m "create_task_response" -v "@id" "%TEMP%\openvas_scan_task_%Timestamp%.txt" > %TEMP%\openvas_scan_task_id_%Timestamp%.txt"
if not %ERRORLEVEL% EQU 0 ( echo ---Error--- && pause && exit )
...
if not %ERRORLEVEL% EQU 0 ( echo ---Error--- && pause && exit )
"%~dp0xml.exe" sel -t -v "start_task_response/report_id" "%TEMP%\openvas_scan_report_%Timestamp%.txt" > %TEMP%\openvas_scan_report_id_%Timestamp%.txt"
...
"%~dp0xml.exe" sel -t -v "get_tasks_response/task/status" "%TEMP%\openvas_scan_status_report_%Timestamp%.txt" > "%TEMP%\openvas_scan_status_scan_%Timestamp%.txt"
...
if %ERRORLEVEL% EQU 0 ( echo: ) else ( ping -n 61 127.0.0.1 > NUL && time /T && goto :while1 )

echo Generando Reporte...
rem http://docs.greenbone.net/API/OMP/omp.html#command_get_reports
"%~dp0omp_cracked.exe" --host=%Server% --port=%Port% --username=%Username% --password=%Password% --xml="<get_reports report_id=\"%report_id%\" filt_id=\"%Filter_id%\" format_id=\"%format_id%\"/>" 1> "%TEMP%\openvas_scan_report_response_%Timestamp%.txt"
"%~dp0xml.exe" sel -t -v get_reports_response/report/text() "%TEMP%\openvas_scan_report_response_%Timestamp%.txt" > "%TEMP%\openvas_scan_b64_report_%Timestamp%.txt"
"%~dp0b64.exe" -d "%TEMP%\openvas_scan_b64_report_%Timestamp%.txt" %Documentacion%
